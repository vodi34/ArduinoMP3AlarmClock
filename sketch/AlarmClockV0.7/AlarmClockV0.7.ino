...
static int volMusic = 20;
static int alarmMusic = 15;
...
void setup(void) {
    ...
    myDFPlayer.volume(volMusic);   //Set volume value. From 0 to 30
    /*
    ...
    */
    ...
    vol_btn.initButton( &tft, 320, 295, 80, 40, BLACK, GREY, BLACK, "", 2);
...
void loop(void)  {  
  ...
  if( down ) {    
    ...
    if( on_btn.contains(pixel_x, pixel_y) > 0 ) {
     ...
     // prüfen ob auf volumnSet button gedrückt wurde
     else if( vol_btn.contains(pixel_x, pixel_y) > 0 ) {
       Serial.println( "volumn button pressed" );
       setVolumn();  // start Volumn-SET Dialog
       showVolumn();
     ...
     // Single press; not pressing any button
        ...
        Serial.println( "Player Mode: touch point received to start music" );
        myDFPlayer.play();
        musicON = true;
        delay(500);
     }
...
/*
 * zeigt unten rechts die Lautstärke an
 * 
 */
void showVolumn() {
    String btnText = "vol: " + String( volMusic );
    // Button is GREY and Font Color Black
    tft.setFont(&FreeSerif12pt7b);
    tft.setTextColor(BLACK);  
    vol_btn.drawButton(false);
    showmsgXY( 286, 300, 1, btnText );    
    tft.setTextColor(WHITE);  
    
    Serial.print( "showVolumn: " + btnText );    
}

/*
 * zeigt die Alarm zeit auf dem Button an
 */
void showAlarm() {
    String alarmString = "";
    String btnText = "";
...
/*
 * zeigt die Uhrzeit in grossen Zahlen an
 */
void showTime() {
...
/*
 * Dialog zum setzen der Lautstärke
 */
void setVolumn() {

  // wenn music aus, dann an um
  // laustärke auch hören zu können
  if( !musicON ) {
    musicON = true;
    myDFPlayer.play(); 
  }
  ...
  // 2 Buttons 
  Adafruit_GFX_Button btn_U;
  btn_U.initButton( &tft,   80, 110, 70, 70, WHITE, GREY, BLACK, "", 2);  // Hour Up
  Adafruit_GFX_Button btn_D;
  btn_D.initButton( &tft,   80, 220, 70, 70, WHITE, GREY, BLACK, "", 2);  // Hour Down

  btn_U.drawButton(false);
  btn_D.drawButton(false);
  ...
  while (true) {
    change = false;
    if( Touch_getXY() ) {
      if(btn_U.contains(pixel_x, pixel_y) > 0 ) {
       volMusic ++;
       if( volMusic == 31 ) volMusic = 0;
       change = true;
      }
      else if(btn_D.contains(pixel_x, pixel_y) > 0) {
       volMusic--;
       if( volMusic == -1 ) { volMusic = 30;  } 
       change = true;
      }
      else if( btn_Exit.contains(pixel_x,pixel_y) > 0 ){
        Serial.println( "exit setVolumn" );
        delay( 1000 );
        break;
      }
      else {
    ...
    String V;
    if( volMusic < 10 ) V = "0"; else V = "";
    V+= String(volMusic);
    tft.setFont(&FreeSevenSegNumFont);
    showmsgXY(140, 180, 1, V );
    myDFPlayer.volume(volMusic);   //Set volume value. From 0 to 30

    delay(75);
    
    if( change ) {
      Serial.print( "setVolumn: " ); Serial.print( V ) ;
    }
  }
...
/*
 * Dialog zum Setzen des Alarms
 * 
 * Bildschirm wird auf weiss gesetzt
 * die grauen +/- sind Buttons
 * die grose Zahl ist auch ein Button zum Verlassen des Dialogs
 * 
 */
void setAlarm() {
  tft.fillScreen(WHITE); // löscht den Screen komplett
  ...
  btn_HU.initButton( &tft,   80, 110, 70, 70, WHITE, GREY, BLACK, "", 2);  // Hour Up
  Adafruit_GFX_Button btn_HD;
  btn_HD.initButton( &tft,   80, 220, 70, 70, WHITE, GREY, BLACK, "", 2);  // Hour Down
  Adafruit_GFX_Button btn_MU;
  btn_MU.initButton( &tft,  350, 110, 70, 70, WHITE, GREY, BLACK, "", 2);  // Minute Up
  Adafruit_GFX_Button btn_MD;
  btn_MD.initButton( &tft,  350, 220, 70, 70, WHITE, GREY, BLACK, "", 2);  // Minute Down
